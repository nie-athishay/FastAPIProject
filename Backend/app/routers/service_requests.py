...
def create_ticket(
    ...
    audit_logs_collection.insert_one(
        build_audit_log_doc(
            ...
            details=f"Ticket created with status '{ServiceRequestStatus.NEW.value}'.",
        )
    )
    return ticket_doc
...
def list_tickets(
    ...
    status_filter: Optional[ServiceRequestStatus] = Query(default=None, alias="status", description="Filter by exact status"),
...
@router.get("/{ticket_id}", response_model=ServiceRequestResponse)
def get_ticket(
    ticket_id: str,
    tickets_collection: Collection = Depends(get_service_request_collection),
):
...
@router.put("/{ticket_id}", response_model=ServiceRequestResponse)
def update_ticket(
    ticket_id: str,
...
@router.patch("/{ticket_id}/assign", response_model=ServiceRequestResponse)
def assign_ticket(
    ticket_id: str,
    ...
    *reassigned* later on (already past NEW), we only change the technician
    ...
    status_also_changed = existing["status"] == ServiceRequestStatus.NEW
    ...
    if status_also_changed:
        details += f" Status moved from '{ServiceRequestStatus.NEW.value}' to '{ServiceRequestStatus.ASSIGNED.value}'."
    audit_logs_collection.insert_one(
        build_audit_log_doc(
            ticket_id=ticket_id,
...
@router.patch("/{ticket_id}/status", response_model=ServiceRequestResponse)
